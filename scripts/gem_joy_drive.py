...
JOY_AXES_CROSS_LR = 6 # Steering (-1 -> 1)
JOY_AXES_CROSS_UD = 7
...
class GemDrive():
    ...
    def joy_callback(self, data):
        ...
        if( data.buttons[JOY_BUTTON_R] != 1.0 ):
            ...
            throttle_cmd = 0.0
            if( self.throttle_initialized ):
                throttle_cmd = (1.0 - data.axes[JOY_TRIGGER_R] ) / 2.0
                throttle_cmd = self.throttle_gain * throttle_cmd
                
            # Check for braking input
            brake_cmd = 0.0
            if( self.brake_initialized ):
                brake_cmd = (1.0 - data.axes[JOY_TRIGGER_L] ) / 2.0
                brake_cmd = self.brake_gain * brake_cmd

            # Check for steering input
            steering_cmd = data.axes[JOY_AXES_RIGHT_STICK_LR]
            steering_cmd = self.steering_gain*steering_cmd*SteeringCmd.TORQUE_MAX
    ...
    # Report callbacks not being used....
